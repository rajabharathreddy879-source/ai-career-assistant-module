...
import { sendChatMessageSchema, createChatSessionSchema } from "../../shared/schemas";
...
const getSessionHandler = async (req: any, res: any): Promise<void> => {
  ...
  try {
    ...
    const [session] = await db.select().from(chatSessionsTable).where(queryCond);
...
const deleteSessionHandler = async (req: any, res: any): Promise<void> => {
  ...
  try {
    ...
    const [deleted] = await db.delete(chatSessionsTable).where(queryCond).returning();
...
router.delete("/history", async (req: any, res: any): Promise<void> => {
  ...
  try {
    await db.delete(chatSessionsTable).where(eq(chatSessionsTable.user_id, userId));
    res.sendStatus(204);
  } catch (err) {
